...
fn batch_update(form_id: String, request: BatchUpdateFormRequest) -> Result<BatchUpdateFormResponse, ()> {
    Err(())
}

/// Request body for `batch_update`.
struct BatchUpdateFormRequest {
...
struct BatchUpdateFormResponse {
...
/// 
/// * `request` - Request body.
fn create(request: Form) -> Result<Form, ()> {
    Err(())
}
...
/// 
/// * `form_id` - Required. The form ID.
fn get(form_id: String) -> Result<Form, ()> {
    Err(())
}
